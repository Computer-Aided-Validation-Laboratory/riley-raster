...
pub const sphere = @import("test_gold_sphere.zig");
pub const nodal_normals = @import("test_nodal_normals.zig");

pub fn main() !void {
    std.debug.print("Running ALL SIMD Gold Test Suites...\n", .{});
    
    std.debug.print("Please use 'zig test -lc -O ReleaseSafe src-simd/test_gold_all.zig' " ++
                   "to run all tests.\n", .{});
}
