        ...
        fn rasterIncremental(
            ...
            for (bounds.start_y..bounds.end_y) |scratch_y| {
                ...
                if (has_geom_simd) {
                    ...
                    var dwdx_simd: [N]@Vector(L, f64) = undefined;
                    inline for (0..N) |nn| {
                        dwdx_simd[nn] = @splat(dweights_dx[nn] * @as(f64, @floatFromInt(L)));
                    }

                    var scratch_x = bounds.start_x;
                    while (scratch_x + L <= bounds.end_x) : (scratch_x += L) {
                        const mask_in = Geometry.isInElementSIMD(weights_simd);
                        if (@reduce(.Or, mask_in)) {
                            ...
                            const mask_z = inv_z_simd >= depth_simd;
                    ...
                    for (scratch_x..bounds.end_x) |cur_scratch_x| {
                        if (Geometry.isInElement(weights)) {
                            ...
                            const index = row_offset + cur_scratch_x;
