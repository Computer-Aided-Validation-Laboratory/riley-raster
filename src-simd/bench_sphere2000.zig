...
pub fn main() !void {
    ...
    const out_dir_base = "out-simd-bench-sphere2000";
    ...
    const report_name = try std.fmt.allocPrint(allocator, "out-simd-bench-sphere2000/bench_{s}.md", .{date});
    defer allocator.free(report_name);
    
    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-simd-bench-sphere2000", .default_dir) catch |err| if (err != error.PathAlreadyExists) return err;
