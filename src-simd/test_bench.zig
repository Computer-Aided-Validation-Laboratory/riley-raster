...
test "Unified SIMD Benchmark Tests" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .out_dir = "out-simd-bench-norm-old-fullraster",
        },
        .{
            .name = "geom",
            .gold_dir = "gold-bench-fullscreen",
            .out_dir = "out-simd-bench-norm-old-geom",
        },
        .{
            .name = "sphere200",
    ...
    std.debug.print("Running Unified SIMD Benchmark Tests...\n", .{});
                            ...
                            if (g_arr_res) |g_arr| {
                                ...
                                if (diff_count == 0) {
                                    ...
                                    const fail_case_prefix = try std.fmt.allocPrint(
                                        allocator, "{s}_{s}", .{ cc.name, case_name }
                                    );
                                    defer allocator.free(fail_case_prefix);

                                    // Save Actual Raw CSV
                                    const act_csv_name = try std.fmt.allocPrint(
                                        allocator, "{s}_actual.csv", .{fail_case_prefix}
                                    );
                                    defer allocator.free(act_csv_name);
                                    var act_mat = MatSlice(f64).init(
                                        t_mut.elems, t_mut.dims[1], t_mut.dims[2] * channels
                                    );
                                    try act_mat.saveCSV(io, fails_dir, act_csv_name);

                                    // Save Actual Scaled BMP
                                    const act_bmp_name = try std.fmt.allocPrint(
                                        allocator, "{s}_actual.bmp", .{fail_case_prefix}
                                    );
                                    defer allocator.free(act_bmp_name);
                                    try iio.saveBMP(io, fails_dir, act_bmp_name, &t_mut, 0, .{
                                        .format = .bmp, .bits = 8,
                                        .scaling = .auto, .channels = channels,
                                    });
                                    ...
                                    const diff_csv_name = try std.fmt.allocPrint(
                                        allocator, "{s}_diff.csv", .{fail_case_prefix}
                                    );
                                    defer allocator.free(diff_csv_name);
                                    var diff_mat = MatSlice(f64).init(
                                        diff_arr.elems, diff_arr.dims[1],
                                        diff_arr.dims[2] * channels
                                    );
                                    try diff_mat.saveCSV(io, fails_dir, diff_csv_name);

                                    const diff_bmp_name = try std.fmt.allocPrint(
                                        allocator, "{s}_diff.bmp", .{fail_case_prefix}
                                    );
    ...
    if (total_fails == 0) {
        std.debug.print("\nALL UNIFIED SIMD BENCHMARK TESTS PASSED!\n", .{});
    } else {
        std.debug.print(
