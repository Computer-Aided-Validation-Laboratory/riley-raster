...
pub fn main() !void {
    ...
    std.debug.print("Starting Full Raster SIMD2 Benchmark ({d}x{d}, {d} run per case)...\n", .{ pixel_num[0], pixel_num[1], runs });
    ...
    try common.writeBenchmarkReport(allocator, io, "Full Raster SIMD2 Benchmark Results", out_dir_base, pixel_num, stats_list.items, max_name_len);
}
