...
pub fn main() !void {
    ...
    const report_name = try std.fmt.allocPrint(allocator, "out-simd-bench-fullraster/bench_{s}.md", .{date});
    defer allocator.free(report_name);
    
    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-simd-bench-fullraster", .default_dir) catch |err| if (err != error.PathAlreadyExists) return err;
