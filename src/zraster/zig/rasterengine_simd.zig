...
pub fn rasterScene(
    ...
    const subpx_inv_z_scratch = try allocator.alignedAlloc(
        f64,
        alignment,
    ...
    const subpx_mask_scratch = try allocator.alignedAlloc(
        bool,
        alignment,
        subpx_tile_total_padded + 8,
    );
    ...
    const subpx_xi_scratch = try allocator.alignedAlloc(
        f64,
        alignment,
        subpx_tile_total_padded + 8,
    );
    ...
    const subpx_eta_scratch = try allocator.alignedAlloc(
        f64,
        alignment,
        subpx_tile_total_padded + 8,
    );
    ...
    const subpx_img_mem = try allocator.alignedAlloc(
        f64,
        alignment,
        (subpx_tile_total_padded + 8) * fields_num,
    );
    ...
    const touched_min_x = try allocator.alloc(usize, subpx_tile_size);
    defer allocator.free(touched_min_x);

    const touched_max_x = try allocator.alloc(usize, subpx_tile_size);
    defer allocator.free(touched_max_x);
...
pub fn RasterPass(
    ...
    const PreparedShader = switch (ShaderData) {
        ...
        sops.TexInput(u8, 1), sops.TexPrepared(u8, 1) => blk: {
            break :blk sops.TexPrepared(u8, 1);
        },
        sops.TexInput(u16, 1), sops.TexPrepared(u16, 1) => blk: {
            break :blk sops.TexPrepared(u16, 1);
        },
        sops.TexInput(u8, 3), sops.TexPrepared(u8, 3) => blk: {
            break :blk sops.TexPrepared(u8, 3);
        },
        sops.TexInput(u16, 3), sops.TexPrepared(u16, 3) => blk: {
            break :blk sops.TexPrepared(u16, 3);
        },
        else => ShaderData,
    };
    ...
    return struct {
            ...
            for (bounds.start_y..bounds.end_y) |scratch_y| {
                ...
                while (scratch_x < bounds.end_x) : (scratch_x += 8) {
                    ...
                    const v_x_mask = (v_scratch_x + v_07 >=
                        ...
                        (v_scratch_x + v_07 < @as(@Vector(S, usize), @splat(bounds.end_x)));
        ...
        fn rasterNewtonSIMD(
            ...
            for (bounds.start_y..bounds.end_y) |scratch_y| {
                const row_offset = scratch_y * domain.tile_size;
                const mask_start = row_offset + bounds.start_x;
                const mask_end = row_offset + bounds.end_x;
                @memset(
            ...
            const v_px_min: @Vector(S, f64) =
                @splat(@as(f64, @floatFromInt(target.tile.x_px_min)));
            const v_step: @Vector(S, f64) = @splat(domain.step);
            const v_05: @Vector(S, f64) = @splat(0.5);
            const v_original_start_x: @Vector(S, usize) = @splat(original_start_x);
            const v_bounds_end_x: @Vector(S, usize) = @splat(bounds.end_x);
...
pub fn resolveScratchDirect(
    ...
    for (0..curr_tile_size_y) |ty| {
        ...
        for (0..curr_tile_size_x) |tx| {
            ...
            const scratch_flat_ind = scratch_row_offset + tx;

            if (fields_num == 1) {
                image_out_arr.set(
                    &[_]usize{ 0, image_px_y, image_px_x },
                    spx_image_scratch.get(0, scratch_flat_ind),
                );
                ...
                image_out_arr.set(
                    ...
                    spx_image_scratch.get(1, scratch_flat_ind),
                );
                image_out_arr.set(
                    &[_]usize{ 2, image_px_y, image_px_x },
                    spx_image_scratch.get(2, scratch_flat_ind),
                );
            } else {
                for (0..fields_num) |ff| {
                    image_out_arr.set(
                        &[_]usize{ ff, image_px_y, image_px_x },
                        spx_image_scratch.get(ff, scratch_flat_ind),
                    );
                }
            }
        }
            ...
            if (fields_num == 1) {
                ...
                for (0..sub_samp) |sy| {
                    ...
                    for (0..sub_samp) |sx| {
                        ...
                        field_sum_0 += spx_image_scratch.get(0, scratch_flat_ind);
                    }
                }
                ...
                for (0..sub_samp) |sy| {
                    ...
                    for (0..sub_samp) |sx| {
                        const scratch_flat_ind: usize = scratch_row_offset + spx_start_x + sx;
                        field_sum_0 += spx_image_scratch.get(0, scratch_flat_ind);
                        field_sum_1 += spx_image_scratch.get(1, scratch_flat_ind);
                        field_sum_2 += spx_image_scratch.get(2, scratch_flat_ind);
                    }
                }
