...
pub fn main() !void {
    ...
    for (mesh_types) |mt| {
        for (shader_types) |st| {
            ...
            for (0..runs) |r| {
                ...
                const data_dir = try std.fmt.bufPrint(&data_dir_buf, "data-bench/{s}_bal1000", .{@tagName(mt)});
    ...
    try common.writeBenchmarkReport(allocator, io, "Balanced 1000 Benchmark Results", out_dir_base, pixel_num, stats_list.items, max_name_len);
}
