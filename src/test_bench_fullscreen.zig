...
pub fn main() !void {
    ...
    inline for (bench_types) |bt| {
        ...
        const out_dir_base = "out-bench-" ++ bt;
        ...
        inline for (mesh_types) |mt| {
            inline for (shader_types) |st| {
                ...
                const suffix = if (is_rgb) "_2" else "";
                
                const bench_csv = try std.fmt.allocPrint(allocator, "{s}/{s}/frame_0_field_0{s}.csv", .{out_dir_base, case_name, suffix});
                defer allocator.free(bench_csv);
                const gold_csv = try std.fmt.allocPrint(allocator, "{s}/{s}/frame_0_field_0{s}.csv", .{gold_dir_base, case_name, suffix});
                defer allocator.free(gold_csv);

                // 3. Load and Compare
                const b_arr_res = common.loadNDArrayFromCSV(allocator, io, bench_csv, channels, false);
                if (b_arr_res) |b_arr| {
                    var b_mut = b_arr;
                    defer { allocator.free(b_mut.elems); b_mut.deinit(allocator); }

                    const g_arr_res = common.loadNDArrayFromCSV(allocator, io, gold_csv, channels, false);
                    if (g_arr_res) |g_arr| {
                        var g_mut = g_arr;
                        defer { allocator.free(g_mut.elems); g_mut.deinit(allocator); }
                    ...
                    std.debug.print("BENCH LOAD ERROR: {s} ({s})\n", .{bench_csv, @errorName(err)});
                    total_fails += 1;
                }
            }
        }
