...
pub fn main() !void {
    ...
    const out_dir_base = "out-bench-bal";
    ...
    std.debug.print("Starting Balanced Benchmark ({d}x{d}, {d} run per case)...\n", 
    ...
    inline for (mesh_types) |mt| {
        inline for (shader_types) |st| {
            ...
            for (0..runs) |r| {
                const data_dir = comptime "data-bench/" ++ @tagName(mt) ++ "_bal";
    ...
    const report_name = try std.fmt.allocPrint(allocator, 
                                               "out-bench-old-bal/bench_{s}.md", 
                                               .{date});
    defer allocator.free(report_name);
    
    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-bench-old-bal", .default_dir) catch |err| 
    ...
    try writer.print("# Balanced Benchmark Results\n", .{});
