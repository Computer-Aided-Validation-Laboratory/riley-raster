...
fn shadeFlat(
    ...
    for (scratch_start_y..scratch_end_y) |yy| {
        ...
        for (scratch_start_x..scratch_end_x) |xx| {
            ...
            var u: f64 = 0.0;
            var v: f64 = 0.0;
            if (newton.solveInverse(N, txs, tys, node_coords.x, node_coords.y, 
                                    node_coords.z, u, v, &u, &v)) {
                var node_weights: [N]f64 = undefined;
                var dNu: [N]f64 = undefined;
                var dNv: [N]f64 = undefined;
                shapeFunctions9(u, v, &node_weights, &dNu, &dNv);

                var sw: f64 = 0.0;
                inline for (0..N) |i| {
                    sw += node_weights[i] * node_coords.z[i];
                }
                const inv_z = 1.0 / sw;
