...
pub fn main() !void {
    ...
    const out_dir_base = "out-bench-bal100";
    ...
    for (mesh_types) |mt| {
        for (shader_types) |st| {
            ...
            for (0..runs) |r| {
                ...
                const data_dir = try std.fmt.bufPrint(&data_dir_buf, "data-bench/{s}_bal100", .{@tagName(mt)});
    ...
    try common.writeBenchmarkReport(allocator, io, "Balanced 100 Benchmark Results", out_dir_base, pixel_num, stats_list.items, max_name_len);
}
