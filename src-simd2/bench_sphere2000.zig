...
pub fn main() !void {
    ...
    const out_dir_base = "out-simd2-bench-sphere2000";
    ...
    inline for (mesh_types) |mt| {
        inline for (shader_types) |st| {
            ...
            for (0..runs) |r| {
                const data_dir = comptime "data-bench/" ++ @tagName(mt) ++ "_sphere2000";
    ...
    const report_name = try std.fmt.allocPrint(allocator, "out-simd2-bench-sphere2000/bench_{s}.md", .{date});
    defer allocator.free(report_name);
    
    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-simd2-bench-sphere2000", .default_dir) catch |err| if (err != error.PathAlreadyExists) return err;
    ...
    try writer.print("# Sphere 2000 Benchmark Results\n", .{});
