...
pub fn rasterElems(allocator: std.mem.Allocator,
    ...
    var spx_image_scratch = MatSlice(f64).init(spx_image_scratch_mem, spx_tile_total, 
                                               fields_num);
    ...
    for (active_tiles) |tile| {
        ...
        for (overlaps) |overlap| {
            const nodes_rast = try rops.loadVec3SlicesFromElemArray(N, f64, elem_coord_arr, 
                                                                    overlap.elem_ind);
            const x0 = nodes_rast.x[0]; const y0 = nodes_rast.y[0]; const z0 = nodes_rast.z[0];
            const x1 = nodes_rast.x[1]; const y1 = nodes_rast.y[1]; const z1 = nodes_rast.z[1];
            const x2 = nodes_rast.x[2]; const y2 = nodes_rast.y[2]; const z2 = nodes_rast.z[2];
            ...
            for (0..fields_num) |ff| {
                f_inv_z_scaled_buffer[ff * N + 0] = elem_field_arr.get(
                    &[_]usize{ overlap.elem_ind, ff, 0 }) * inv_z0;
                f_inv_z_scaled_buffer[ff * N + 1] = elem_field_arr.get(
                    &[_]usize{ overlap.elem_ind, ff, 1 }) * inv_z1;
                f_inv_z_scaled_buffer[ff * N + 2] = elem_field_arr.get(
                    &[_]usize{ overlap.elem_ind, ff, 2 }) * inv_z2;
            }
                                ...
                                if (v_inv_z[l] > spx_inv_z_scratch[idx]) {
                                    ...
                                    for (0..fields_num) |ff| {
                                        ...
                                        spx_image_scratch.elems[img_row + ff] = 
                                            (w0_val * f0 + w1_val * f1 + w2_val * f2) * z_val;
                                    }
                                }
                            }
                        }
