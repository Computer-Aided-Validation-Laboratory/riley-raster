...
pub fn rasterElems(
    ...
    const field_node_term_splats = try allocator.alloc(
        @Vector(v_len, f64),
        fields_num * N,
    );
    ...
    for (active_tiles) |tile| {
        ...
        for (overlaps) |overlap| {
            ...
            for (0..fields_num) |ff| {
                ...
                inline for (0..N) |nn| {
                    ...
                    const term = elem_field_arr.get(inds[0..]) *
                        nodes_inv_z[nn] *
                        inv_area;
            ...
            const v_n_inv_z0 = @as(@Vector(v_len, f64), @splat(
                nodes_inv_z[0] * inv_area,
            ));
            const v_n_inv_z1 = @as(@Vector(v_len, f64), @splat(
                nodes_inv_z[1] * inv_area,
            ));
            const v_n_inv_z2 = @as(@Vector(v_len, f64), @splat(
                nodes_inv_z[2] * inv_area,
            ));
        ...
        const spx_image_scratch = MatSlice(f64).init(
            spx_image_scratch_mem,
            spx_tile_total,
            fields_num,
        );
