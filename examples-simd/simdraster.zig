...
pub fn countElemsCalcBBoxes(camera: *const Camera, dim_elem: usize, elem_coord_arr: *const NDArray(f64), elem_bboxes: []BBox) !usize {
    const N: usize = 3;
...
pub fn rasterElems(allocator: std.mem.Allocator, camera: *const Camera, tile_size: u16, active_tiles: []ActiveTile, overlap_bboxes: []BBox, elem_coord_arr: *const NDArray(f64), elem_field_arr: *const NDArray(f64), image_out_arr: *NDArray(f64)) !void {
    ...
    for (active_tiles) |tile| {
        ...
        for (overlaps) |overlap| {
            ...
            var w0_row = rops.edgeFun3(nodes_rast.x[1], nodes_rast.y[1], nodes_rast.x[2], nodes_rast.y[2], start_x, start_y);
            var w1_row = rops.edgeFun3(nodes_rast.x[2], nodes_rast.y[2], nodes_rast.x[0], nodes_rast.y[0], start_x, start_y);
            var w2_row = rops.edgeFun3(nodes_rast.x[0], nodes_rast.y[0], nodes_rast.x[1], nodes_rast.y[1], start_x, start_y);
            ...
            const v_dw0_step = @as(
                @Vector(v_len, f64),
                @splat(dw0_dx * @as(f64, @floatFromInt(v_len))),
            );
            const v_dw1_step = @as(
                @Vector(v_len, f64),
                @splat(dw1_dx * @as(f64, @floatFromInt(v_len))),
            );
            const v_dw2_step = @as(
                @Vector(v_len, f64),
                @splat(dw2_dx * @as(f64, @floatFromInt(v_len))),
            );
        ...
        const spx_image_scratch = MatSlice(f64).init(spx_image_scratch_mem, spx_tile_total, fields_num);
        const spx_field_avg = try allocator.alloc(f64, fields_num);
        defer allocator.free(spx_field_avg);

        rops.averageScratch(tile, tile_size, screen_w, screen_h, sub_samp, spx_tile_size, fields_num, &spx_image_scratch, spx_field_avg, image_out_arr);
    }
}

pub fn rasterFrame(allocator: std.mem.Allocator, io: std.Io, frame_ind: usize, coords: *const Coords, connect: *const Connect, field: *const Field, camera: *const Camera, image_out_arr: *NDArray(f64)) !void {
    ...
    var elem_coord_arr = try rops.initElemArray(f64, arena_alloc, elems_num, coords_num, nodes_per_elem);
    var elem_field_arr = try rops.initElemArray(f64, arena_alloc, elems_num, fields_num, nodes_per_elem);
    ...
    const elems_in_image = try countElemsCalcBBoxes(camera, dim_elem, &elem_coord_arr, elem_bboxes);
    ...
    const num_active_tiles = try rops.elemTileOverlapCount(tile_size, tiles_num_x, elems_in_image, elem_bboxes, tile_elem_counts, tile_write_inds);
    ...
    rops.storeActiveTiles(tile_size, tiles_num_x, tiles_num_y, screen_px_x, screen_px_y, elems_in_image, elem_bboxes, tile_elem_counts, tile_write_inds, overlap_bboxes, active_tiles);
    ...
    try rasterElems(arena_alloc, camera, tile_size, active_tiles, overlap_bboxes, &elem_coord_arr, &elem_field_arr, image_out_arr);
