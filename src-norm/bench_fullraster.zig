...
pub fn main() !void {
    ...
    const out_dir_base = "out-norm-fullraster";
    ...
    const report_name = try std.fmt.allocPrint(allocator, "out-norm-fullraster/bench_{s}.md", .{date});
    defer allocator.free(report_name);
    
    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-norm-fullraster", .default_dir) catch |err| if (err != error.PathAlreadyExists) return err;
