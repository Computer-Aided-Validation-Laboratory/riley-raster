...
pub fn main() !void {
    ...
    const out_dir_base = "out-norm-geom";
    ...
    const report_name = try std.fmt.allocPrint(allocator, "out-norm-geom/bench_{s}.md", .{date});
    defer allocator.free(report_name);
    
    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-norm-geom", .default_dir) catch |err| if (err != error.PathAlreadyExists) return err;
