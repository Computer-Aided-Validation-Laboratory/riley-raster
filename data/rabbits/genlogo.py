...
/home/lloydf/lfdev/fonts (Audiowide, Goldman, Michroma, Prosto One, Zen Dots).
Because text is converted to native SVG vector paths, the fonts render 100%
identically in any viewer, browser, or editor without font dependencies.
"""
...
FONT_DEFAULT_FAMILY: str = "Audiowide"
...
FONT_PATHS: dict[str, Path] = {
    ...
    "Goldman Bold": FONTS_DIR / "Goldman" / "Goldman-Bold.ttf",
...
def build_logo_elements(
    ...
    if font_path and font_path.exists():
        ...
        svg_items.append(f'<path d="{d_path}" fill="{font_color}" />')
    else:
        svg_items.append(
...
def gen_group1_var1(out_dir: Path, base_svg: Path) -> None:
    """G1.1: Style 1 (N=76, Med), Audiowide 80pt, Emerald Obsidian."""
    ...
    elems = build_logo_elements(
        ...
        font_family="Audiowide", font_size=80.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_01_var1_emerald_audiowide_80pt.svg",
        w, h, "#070b0e", elems
    )


def gen_group1_var2(out_dir: Path, base_svg: Path) -> None:
    """G1.2: Style 1 (N=84, Med-Fine), Goldman Bold 82pt."""
    ...
    elems = build_logo_elements(
        ...
        font_family="Goldman Bold", font_size=82.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_01_var2_emerald_goldman_bold_82pt.svg",
        w, h, "#070b0e", elems
    )


def gen_group1_var3(out_dir: Path, base_svg: Path) -> None:
    """G1.3: Style 1 (N=80, Med), Michroma 72pt (wide technical)."""
    ...
    elems = build_logo_elements(
        ...
        stroke_color="#10b981", stroke_width=1.2,
        vertex_color="#a7f3d0", vertex_radius=2.0,
        font_family="Michroma", font_size=72.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_01_var3_emerald_michroma_72pt.svg",
        w, h, "#070b0e", elems
    )


def gen_group1_var4(out_dir: Path, base_svg: Path) -> None:
    """G1.4: Style 1 (N=92, Fine), Zen Dots 78pt."""
    ...
    elems = build_logo_elements(
        ...
        font_family="Zen Dots", font_size=78.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_01_var4_emerald_zen_dots_78pt.svg",
        w, h, "#070b0e", elems
    )


def gen_group1_var5(out_dir: Path, base_svg: Path) -> None:
    """G1.5: Style 1 (N=88, Med-Fine), Prosto One 84pt."""
    ...
    elems = build_logo_elements(
        ...
        stroke_color="#10b981", stroke_width=1.1,
        vertex_color="#a7f3d0", vertex_radius=1.8,
        font_family="Prosto One", font_size=84.0, font_color="#ffffff",
    )
    render_svg_file(
...
def gen_group1_var6(out_dir: Path, base_svg: Path) -> None:
    """G1.6: Style 1 (N=104, Superfine), Goldman Regular 80pt."""
    ...
    elems = build_logo_elements(
        ...
        font_family="Goldman", font_size=80.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_01_var6_emerald_goldman_superfine_80pt.svg",
        w, h, "#070b0e", elems
    )
...
def gen_group2_var1(out_dir: Path, base_svg: Path) -> None:
    """G2.1: Style 5 (N=88, Med-Fine), Audiowide 80pt, Precision CAD."""
    boundary = load_rabbit_boundary(base_svg, num_points=88)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.020)
    svg_pts = transform_coords(coords, scale, ox, oy)
    ...
    render_svg_file(
        out_dir / "logo_05_var1_cad_audiowide_80pt.svg",
        w, h, "#090f14", elems
    )


def gen_group2_var2(out_dir: Path, base_svg: Path) -> None:
    """G2.2: Style 5 (N=96, Fine), Goldman Bold 84pt."""
    ...
    elems = build_logo_elements(
        ...
        font_family="Goldman Bold", font_size=84.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_05_var2_cad_goldman_bold_84pt.svg",
        w, h, "#090f14", elems
    )


def gen_group2_var3(out_dir: Path, base_svg: Path) -> None:
    """G2.3: Style 5 (N=84, Med), Michroma 74pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=84)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.022)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        font_family="Michroma", font_size=74.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_05_var3_cad_michroma_74pt.svg",
        w, h, "#090f14", elems
    )


def gen_group2_var4(out_dir: Path, base_svg: Path) -> None:
    """G2.4: Style 5 (N=104, Superfine), Zen Dots 80pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=104)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.017)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        font_family="Zen Dots", font_size=80.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_05_var4_cad_zen_dots_80pt.svg",
        w, h, "#090f14", elems
    )


def gen_group2_var5(out_dir: Path, base_svg: Path) -> None:
    """G2.5: Style 5 (N=88, Med-Fine), Prosto One 82pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=88)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.020)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        stroke_color="#34d399", stroke_width=1.1,
        vertex_color="#ffffff", vertex_radius=1.8,
        font_family="Prosto One", font_size=82.0, font_color="#ffffff",
    )
    render_svg_file(
...
def gen_group2_var6(out_dir: Path, base_svg: Path) -> None:
    """G2.6: Style 5 (N=92, Fine), Zen Dots with Mint Text 80pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=92)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.019)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        font_family="Zen Dots", font_size=80.0, font_color="#34d399",
    )
    render_svg_file(
        out_dir / "logo_05_var6_cad_zen_dots_mint_80pt.svg",
        w, h, "#090f14", elems
    )
...
def gen_group3_var1(out_dir: Path, base_svg: Path) -> None:
    """G3.1: Style 18 (N=76, Med), Audiowide 82pt, Slate Graphite."""
    boundary = load_rabbit_boundary(base_svg, num_points=76)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.024)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        font_family="Audiowide", font_size=82.0, font_color="#f9fafb",
    )
    render_svg_file(
        out_dir / "logo_18_var1_graphite_audiowide_82pt.svg",
        w, h, "#0c1015", elems
    )


def gen_group3_var2(out_dir: Path, base_svg: Path) -> None:
    """G3.2: Style 18 (N=88, Med-Fine), Goldman Bold 84pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=88)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.021)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        font_family="Goldman Bold", font_size=84.0, font_color="#f9fafb",
    )
    render_svg_file(
        out_dir / "logo_18_var2_graphite_goldman_bold_84pt.svg",
        w, h, "#0c1015", elems
    )


def gen_group3_var3(out_dir: Path, base_svg: Path) -> None:
    """G3.3: Style 18 (N=80, Med), Michroma 72pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=80)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    ...
    render_svg_file(
        out_dir / "logo_18_var3_graphite_michroma_72pt.svg",
        w, h, "#0c1015", elems
    )


def gen_group4_var4(out_dir: Path, base_svg: Path) -> None:
    """G3.4: Style 18 (N=92, Fine), Zen Dots 78pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=92)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.020)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        stroke_color="#10b981", stroke_width=1.1,
        vertex_color="#6ee7b7", vertex_radius=1.8,
        font_family="Zen Dots", font_size=78.0, font_color="#f9fafb",
    )
    render_svg_file(
        out_dir / "logo_18_var4_graphite_zen_dots_78pt.svg",
        w, h, "#0c1015", elems
    )


def gen_group3_var5(out_dir: Path, base_svg: Path) -> None:
    """G3.5: Style 18 (N=104, Superfine), Prosto One 84pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=104)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.017)
    svg_pts = transform_coords(coords, scale, ox, oy)
...
def gen_group3_var6(out_dir: Path, base_svg: Path) -> None:
    """G3.6: Style 18 (N=84, Med-Fine), Goldman with Mint Text 80pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=84)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.022)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        stroke_color="#10b981", stroke_width=1.2,
        vertex_color="#6ee7b7", vertex_radius=1.9,
        font_family="Goldman", font_size=80.0, font_color="#6ee7b7",
    )
    render_svg_file(
        out_dir / "logo_18_var6_graphite_goldman_mint_80pt.svg",
        w, h, "#0c1015", elems
    )
...
def gen_group4_var1(out_dir: Path, base_svg: Path) -> None:
    """G4.1: Style 19 (N=92, Fine), Audiowide 80pt, Density Mesh."""
    boundary = load_rabbit_boundary(base_svg, num_points=92)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.019)
    svg_pts = transform_coords(coords, scale, ox, oy)
    ...
    render_svg_file(
        out_dir / "logo_19_var1_density_audiowide_80pt.svg",
        w, h, "#081014", elems
    )


def gen_group4_var2(out_dir: Path, base_svg: Path) -> None:
    """G4.2: Style 19 (N=104, Superfine), Goldman Bold 86pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=104)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.017)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        stroke_color="#34d399", stroke_width=0.9,
        vertex_color="#ffffff", vertex_radius=1.5,
        font_family="Goldman Bold", font_size=86.0, font_color="#ffffff",
    )
    render_svg_file(
        out_dir / "logo_19_var2_density_goldman_bold_86pt.svg",
        w, h, "#081014", elems
    )


def gen_group4_var3(out_dir: Path, base_svg: Path) -> None:
    """G4.3: Style 19 (N=88, Med-Fine), Michroma 74pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=88)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.020)
    svg_pts = transform_coords(coords, scale, ox, oy)
    ...
    render_svg_file(
        out_dir / "logo_19_var3_density_michroma_74pt.svg",
        w, h, "#081014", elems
    )


def gen_group4_var4_alt(out_dir: Path, base_svg: Path) -> None:
    """G4.4: Style 19 (N=112, Micro-Density), Zen Dots 80pt."""
    ...
    render_svg_file(
        out_dir / "logo_19_var4_density_zen_dots_80pt.svg",
        w, h, "#081014", elems
    )


def gen_group4_var5(out_dir: Path, base_svg: Path) -> None:
    """G4.5: Style 19 (N=96, Fine), Prosto One 84pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=96)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.019)
    svg_pts = transform_coords(coords, scale, ox, oy)
...
def gen_group4_var6(out_dir: Path, base_svg: Path) -> None:
    """G4.6: Style 19 (N=104, Superfine), Audiowide Mint Text 80pt."""
    boundary = load_rabbit_boundary(base_svg, num_points=104)
    w, h, scale, ox, oy = get_tight_viewport(boundary)
    coords, conn = build_triangle_chain_mesh(boundary, ribbon_width=0.017)
    svg_pts = transform_coords(coords, scale, ox, oy)
    elems = build_logo_elements(
        ...
        stroke_color="#34d399", stroke_width=0.9,
        vertex_color="#ffffff", vertex_radius=1.5,
        font_family="Audiowide", font_size=80.0, font_color="#a7f3d0",
    )
    render_svg_file(
        out_dir / "logo_19_var6_density_audiowide_mint_80pt.svg",
        w, h, "#081014", elems
    )
...
def generate_gallery_html(logo_dir: Path) -> None:
    ...
    html_lines = [
        ...
        "  <title>Riley Refined Logo Variations (Vector Outlines)</title>",
        "  <style>",
        "    body {",
        "      margin: 0;",
        "      padding: 30px;",
        ...
        "</head>",
        "<body>",
        "  <h1>Riley Refined Logo Variations (Exact Vector Outlines)</h1>",
        '  <p class="subtitle">24 variations exploring Audiowide, Goldman, '
        "Michroma, Prosto One, and Zen Dots converted to exact vector outlines "
        "across medium to superfine ribbon mesh densities</p>",
        '  <div class="grid">',
    ]
    ...
    for svg_file in svg_files:
        ...
        html_lines.append(f'      <div class="card-header">{title}</div>')
...
def generate_all_logos() -> None:
    ...
    generators = [
        # Group 1: Style 1 (Emerald Obsidian Theme)
        ("01.1 Emerald Audiowide 80pt", gen_group1_var1),
        ("01.2 Emerald Goldman Bold 82pt", gen_group1_var2),
        ("01.3 Emerald Michroma 72pt", gen_group1_var3),
        ("01.4 Emerald Zen Dots 78pt", gen_group1_var4),
        ("01.5 Emerald Prosto One 84pt", gen_group1_var5),
        ("01.6 Emerald Goldman Superfine 80pt", gen_group1_var6),
        # Group 2: Style 5 (Precision CAD Theme)
        ("05.1 CAD Audiowide 80pt", gen_group2_var1),
        ("05.2 CAD Goldman Bold 84pt", gen_group2_var2),
        ("05.3 CAD Michroma 74pt", gen_group2_var3),
        ("05.4 CAD Zen Dots 80pt", gen_group2_var4),
        ("05.5 CAD Prosto One 82pt", gen_group2_var5),
        ("05.6 CAD Zen Dots Mint 80pt", gen_group2_var6),
        # Group 3: Style 18 (Slate Graphite Theme)
        ("18.1 Graphite Audiowide 82pt", gen_group3_var1),
        ("18.2 Graphite Goldman Bold 84pt", gen_group3_var2),
        ("18.3 Graphite Michroma 72pt", gen_group3_var3),
        ("18.4 Graphite Zen Dots 78pt", gen_group4_var4),
        ("18.5 Graphite Prosto One 84pt", gen_group3_var5),
        ("18.6 Graphite Goldman Mint 80pt", gen_group3_var6),
        # Group 4: Style 19 (Ultra-Density Mesh Theme)
        ("19.1 Density Audiowide 80pt", gen_group4_var1),
        ("19.2 Density Goldman Bold 86pt", gen_group4_var2),
        ("19.3 Density Michroma 74pt", gen_group4_var3),
        ("19.4 Density Zen Dots 80pt", gen_group4_var4_alt),
        ("19.5 Density Prosto One 84pt", gen_group4_var5),
        ("19.6 Density Audiowide Mint 80pt", gen_group4_var6),
    ]
