...
- Rejected:
  - The linear-triangle implementation attempt for items `3` and `4`, adding coarse
    accept plus full-coverage and full-depth store fast paths for `tri3` and `tri3opt`,
    passed both required test suites but was a major performance regression.
  - Measured `MPx/s` regressions were broadly in the `-10%` to `-30%` range on the
    targeted `tri3` and `tri3opt` cases.
  - Representative regressions included:
    `fullraster:tri3_flat_grey` about `-32.7%`,
    `fullraster:tri3opt_flat_grey` about `-32.5%`,
    `sphere2000:tri3_flat_grey` about `-26.8%`,
    `sphere2000:tri3opt_flat_grey` about `-26.5%`,
    `geom:tri3_flat_grey` about `-21.7%`,
    `geom:tri3opt_flat_grey` about `-22.6%`.
  - The change also introduced secondary regressions in other kernels because the
    full-lane store branching touched shared SIMD shader paths.
  - Conclusion: revert items `3` and `4` in their current form and preserve the `v2`
    implementation as the active baseline.

- Rejected:
  - Item `5`, tracking dirty regions in scratch to avoid whole-tile clears, is not a
    good fit for the current tile configuration.
  - In the active setup, tiles are `32 x 32` pixels with `sub_sample = 2`, so the
    scratch footprint is only `64 x 64 = 4096` subpixels per tile.
  - At this size, whole-buffer `@memset` of `inv_z` and image scratch is simple,
    predictable, and cache-friendly.
  - Dirty-row or dirty-chunk tracking would add bookkeeping writes, extra branches, and
    more complicated resolve logic for a very small scratch footprint.
  - Conclusion: for the current `src-simd2` design, full scratch clears are cleaner and
    are unlikely to be slower than tracking touched regions.

- Accepted:
  - Item `6`, resolving only dirty output pixels, was accepted as the `v3`
    optimisation.
  - The implementation kept full scratch clears, added per-subpixel-row touched x
    bounds, used an explicit `sub_sample == 1` resolve bypass, and specialized
    `averageScratch` for `fields_num == 1`, `fields_num == 3`, and a generic fallback.
  - This passed both required test suites and materially improved `MPx/s` versus `v2`,
    including strong gains in `tri3`, `tri3opt`, and the nonlinear Newton-family
    kernels because they all benefit from the cheaper resolve stage.

- Rejected:
  - Item `8`, prepacking per-element shader payloads into a more direct raster-ready
    path, was explored in two steps and rejected.
  - Step `1`, replacing indexed `LocalNodeBuffer.load` calls with direct packed-slice
    copies, was test-clean but already slightly negative on `bench_geom`.
  - Step `2`, removing the per-overlap `LocalNodeBuffer` copy and feeding packed field
    and normal slices directly into the shader paths, also passed both required test
    suites but regressed performance versus the accepted `v3` baseline.
  - Measured `MPx/s` changes for step `2` versus `v3` were approximately:
    `fullraster -4.3%` overall and `geom -0.8%` overall.
  - Representative regressions included:
    `fullraster:tri3_flat_rgb -16.8%`,
    `fullraster:tri3_flat_grey -11.2%`,
    `fullraster:tri6_tex8_rgb_linear -10.8%`,
    `fullraster:quad9_tex8_rgb_linear -9.6%`,
    `geom:tri3_tex8_rgb_linear -10.4%`,
    `geom:quad4ibi_flat_grey -10.4%`,
    `geom:quad4newton_tex8_rgb_linear -8.2%`,
    `geom:quad8_tex8_rgb_linear -7.0%`.
  - There were isolated wins in some `tri3opt` flat and cubic-texture cases, but they
    were not enough to offset the broader losses.
  - Conclusion: revert item `8` and keep `v3` as the active checkpoint.
