...
To keep the size of the `Riley` repository small we only include gold reference images for the min test suite for `f64` with SIMD enabled as this is the main production path. The min test suite is specifically not enabled for `f32` and will rasie a compilation error if tested.
...
The `zig build` workflow supports precision and SIMD selection directly. Example commands:
...
where CASE is fullraster (raster loop performance), geom (geometry preprocessor performance), sphere2000 (realistic geometry/balanced case), and sphere2000zoom (realistic case testing all culling functions). We provide representative single threaded performance from an AMD zen4 laptop CPU (Ryzen 7 8845HS) packged with the repo in the  directory.
...
The main entry point for the `Riley` rendering pipeline is the `raster(...)` functions in /src/riley/zig/riley.zig. For an overview of the code we recommend you read our preprint on  engrxiv: [Riley: A computational framework for higher-order finite element image synthesis applied to digital image correlation uncertainty qunatification](https://engrxiv.org/preprint/view/7300/version/9460). For the mathematical foundation of the main algorithms go to section 2 and for an overview of the computational architecture go to section 3.
