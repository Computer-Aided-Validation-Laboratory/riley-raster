...
pub fn Texture(comptime T: type, comptime channels: usize) type {
    ...
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator, rows: usize, cols: usize) !Self {
            const array = try NDArray(f64).initFlat(allocator, &[_]usize{ rows, cols, channels });
            return Self{
                .array = array,
                .rows_num = rows,
                .cols_num = cols,
        ...
        pub fn setVal(self: *Self, ch: usize, row: usize, col: usize, val: f64) void {
            self.array.set(&[_]usize{ row, col, ch }, val);
        }

        pub fn getVal(self: *const Self, ch: usize, row: usize, col: usize) f64 {
            return self.array.get(&[_]usize{ row, col, ch });
        }
...
fn sample2DInnerSIMD(comptime channels: usize,
        ...
        start_y >= 0 and start_y + @as(isize, @intCast(N)) <= rows) 
    {
        ...
        const stride_y = texture.array.strides[0];
        const stride_x = texture.array.strides[1];
        
        const w_sum = @reduce(.Add, v_wx) * @reduce(.Add, v_wy);
        const inv_w_sum = if (@abs(w_sum) > 1e-9) 1.0 / w_sum else 1.0;

        // Use a 4-lane SIMD vector for RGB processing
        const VSize = if (channels <= 4) 4 else channels;
        var v_acc: @Vector(VSize, f64) = @splat(0.0);

        inline for (0..N) |jj| {
            const row_start = @as(usize, @intCast(start_y + @as(isize, @intCast(jj)))) * stride_y;
            const w_y = v_wy[jj];
            inline for (0..N) |ii| {
                const px_off = row_start + @as(usize, @intCast(start_x + @as(isize, @intCast(ii)))) * stride_x;
                const w = v_wx[ii] * w_y;
                
                // Load pixel data (channels are contiguous)
                var v_px: @Vector(VSize, f64) = @splat(0.0);
                const px_slice = texture.array.elems[px_off..px_off+channels];
                inline for (0..channels) |ch| {
                    v_px[ch] = px_slice[ch];
                }
                
                v_acc += v_px * @as(@Vector(VSize, f64), @splat(w));
            }
        }
        
        v_acc *= @splat(inv_w_sum);
        inline for (0..channels) |ch| {
            res[ch] = v_acc[ch];
        }
    } else {
