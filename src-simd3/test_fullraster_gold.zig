...
test "Fullraster Gold Comparison" {
    ...
    std.debug.print("Comparing out-simd3-bench-norm-old-fullraster against Gold data...\n", .{});

    inline for (mesh_types) |mt| {
        inline for (shader_types) |st| {
            const case_name = comptime @tagName(mt) ++ "_" ++ @tagName(st);
            
            const bench_csv = try std.fmt.allocPrint(allocator, "out-simd3-bench-norm-old-fullraster/{s}/frame_0_field_0{s}.csv", .{
                case_name,
