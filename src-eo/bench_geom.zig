...
pub fn main() !void {
    ...
    const report_name = try std.fmt.allocPrint(allocator, "out-eo-geom/bench_{s}.md", .{date});
    defer allocator.free(report_name);

    const cwd = std.Io.Dir.cwd();
    cwd.createDir(io, "out-eo-geom", .default_dir) catch |err| if (err != error.PathAlreadyExists) return err;    const file = try cwd.createFile(io, report_name, .{});
    defer file.close(io);
